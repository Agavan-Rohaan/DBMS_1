--Lab 6 Perform SQL queries for Like operator
--Part – A:
...
SELECT *
FROM STUDENT 
WHERE FIRSTNAME LIKE 'B%' AND LASTNAME LIKE '%A' AND Website LIKE '%[MATH,SCIENCE]%'
...
SELECT * 
FROM STUDENT 
WHERE CITY LIKE '%[^S]%' AND ADDRESS LIKE '%[''""]%' AND LASTNAME LIKE 'P%' AND WEBSITE LIKE '%ON%'